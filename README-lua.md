# lhelper — Lua implementation

This is the rewrite of lhelper in Lua 5.4. The bash implementation is still
present in the repository; the two can coexist until the migration is
complete.

## Layout

- `vendor/lua/` — vendored Lua 5.4 sources (no external dependency).
- `csrc/` — the C layer: `main.c` embeds Lua and runs the main script,
